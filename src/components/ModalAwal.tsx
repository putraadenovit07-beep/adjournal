...
export default function ModalAwalPage({ goals, entries, payouts, onSavePayouts }: Props) {
  ...
  return (
    <div className="page">
      <div className="ph">
        <h1>Modal &amp; Payout</h1>
        <p>Catatan payout Adsense &amp; info modal</p>
      </div>
        ...
        <div className="card-b" style={{ padding: '10px 14px' }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--t1)' }}>
            ...
            <span style={{ color: 'var(--p)' }}>
              {modalNum > 0 ? fRp(modalNum) : 'Belum diatur'}
            </span>
            {goals.modalCurrency === 'usd' && goals.modalUsdAmount && (
              <span style={{ fontSize: 11, opacity: 0.6, marginLeft: 6 }}>
                ≈ ${goals.modalUsdAmount.toFixed(2)}
              </span>
            )}
          </div>
          {modalNum > 0 && (
            <div style={{ fontSize: 11, color: 'var(--t3)', marginTop: 4, fontFamily: "'JetBrains Mono', monospace" }}>
              Terpakai: <span style={{ color: 'var(--r)' }}>{fRp(totalSpend)}</span>
              {' · '}
              Profit: <span style={{ color: netProfit >= 0 ? 'var(--g)' : 'var(--r)' }}>
      ...
      <div className="card">
        ...
        >
          <span className="card-h-title">
            {poEditId ? 'Edit Payout Adsense' : 'Tambah Payout Adsense'}
          </span>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {poEditId && (
              <span
                className="card-h-tag"
                style={{ cursor: 'pointer' }}
                onClick={e => { e.stopPropagation(); resetPoForm(); setPayoutOpen(false); }}
              >
                Batal Edit
              </span>
            )}
        ...
        {payoutOpen && (
          <div className="card-b">
            ...
            <div className="fg">
              ...
              <input
                type="number" placeholder="Contoh: 1500000"
                value={poAmount} onChange={e => setPoAmount(e.target.value)} inputMode="numeric"
              />
            </div>
            <div className="two-col">
              <div className="fg">
            ...
            <div className="btn-row">
              <button
                ...
                disabled={!poDate || !(parseFloat(poAmount) > 0)}
              >
              ...
              {poSavedMsg && (
                <span style={{ fontSize: 12, color: 'var(--g)', fontWeight: 600 }}>✓ Tersimpan</span>
              )}
            </div>
          </div>
        )}
                  ...
                  <div className="po-item-main">
                    ...
                    {(p.bankName || p.accountNo || p.accountHolder) && (
                      <div className="po-item-bank">
                        ...
                        {p.accountHolder ? ` · a.n. ${p.accountHolder}` : ''}
                      </div>
                    )}
